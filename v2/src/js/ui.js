class UI
{
    static configureUI() {
        ...
        // fix contents ont top while scrolling
        window.onscroll = UI.windowOnScroll;
    }

    // on scrolling window
    static windowOnScroll()
    {
        let domHr = $("#hrTopToBottom")[0];
        let coords = domHr.getBoundingClientRect();
        let domContents = $("#contents");

        if (coords.top > 0) {
            if (domContents.hasClass("contentsFixed")) {
                domContents.removeClass("contentsFixed");
            }
        }
        else {
            if (!domContents.hasClass("contentsFixed")) {
                domContents.addClass("contentsFixed");
            }
        }
    }
